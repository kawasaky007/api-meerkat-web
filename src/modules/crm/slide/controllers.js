import Slide from '../../../collections/slide'
...
export const updateItem = asyncHandler(async (req, res, next) => {
  let data = await Slide.findById(req.params.id)
  if (!data) {
    return next(new ErrorResponse(`Không tìm thấy thông tin`, 404))
  }
  data = await Slide.findByIdAndUpdate(req.params.id, { ...req.body });
...
export const deleteItem = asyncHandler(async (req, res, next) => {
  ...
  let item = await Slide.findById(req.params.id)
  if (!item) {
    return next(new ErrorResponse(`Không tìm thấy thông tin`, 404))
  }
